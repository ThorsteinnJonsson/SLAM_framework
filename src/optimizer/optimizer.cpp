...
void Optimizer::BundleAdjustment(const std::vector<KeyFrame*>& keyframes, 
                                 ...
                                 const unsigned long loop_kf_index, 
                                 const bool is_robust) {
  std::vector<bool> vbNotIncludedMP(map_points.size(), false); //TODO vector of bool
  ...
  for(size_t i=0; i<keyframes.size(); i++)
  {
      ...
      else
      {
          ...
          pKF->bundle_adj_global_for_keyframe_id = loop_kf_index;
      }
  }
  ...
  for(size_t i=0; i<map_points.size(); i++)
  {
      ...
      else
      {
          ...
          pMP->bundle_adj_global_for_keyframe_id = loop_kf_index;
      }
  }
}
                                       ...
                                       const bool bFixScale)
{
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend;i++)
    {
        ...
        LoopCloser::KeyFrameAndPose::const_iterator it = CorrectedSim3.find(pKF);
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        LoopCloser::KeyFrameAndPose::const_iterator iti = NonCorrectedSim3.find(pKF);
        ...
        if(pParentKF)
        {
            ...
            LoopCloser::KeyFrameAndPose::const_iterator itj = NonCorrectedSim3.find(pParentKF);
        ...
        for(std::set<KeyFrame*>::const_iterator sit=sLoopEdges.begin(), send=sLoopEdges.end(); sit!=send; sit++)
        {
            ...
            if(pLKF->mnId<pKF->mnId)
            {
                ...
                LoopCloser::KeyFrameAndPose::const_iterator itl = NonCorrectedSim3.find(pLKF);
        ...
        for(std::vector<KeyFrame*>::const_iterator vit=vpConnectedKFs.begin(); vit!=vpConnectedKFs.end(); vit++)
        {
            ...
            if(pKFn && pKFn!=pParentKF && !pKF->hasChild(pKFn) && !sLoopEdges.count(pKFn))
            {
                if(!pKFn->isBad() && pKFn->mnId<pKF->mnId)
                {
                    ...
                    LoopCloser::KeyFrameAndPose::const_iterator itn = NonCorrectedSim3.find(pKFn);
